...
public class TourDiagramFragment_bk extends Fragment {
    private static final String TAG = TourDiagramFragment_bk.class.getName();
    ...
    public static TourDiagramFragment_bk newInstance(Activity mActivity, Tour mTour) {

        Bundle bundle = new Bundle();
        bundle.putSerializable(TOUR, mTour);

        TourDiagramFragment_bk fragment = new TourDiagramFragment_bk();
    ...
    private void updateView(HashMap<String, List<Place>> detailDays, List<Travel> travels) {
        ...
        for (int i = 0; i < numDays; i++) {
           ...
           // tblDayDiagram.setColumnStretchable(1,true);
           // tblDayDiagram.setColumnStretchable(3,true);
            tblDayDiagram.setBackgroundColor(Color.parseColor("#62b0ff"));


            int totalPlacesPerDay = places.size();
            int numRow;
            ...
            Log.d("TY-TEST:", "total places: " + totalPlacesPerDay + ", remainder: " + remainder + ", numRow: " + numRow);
            ...
            strDepartureTime = strStartTime;
            String strPreTime = strStartTime;
            String strNextTime;
                    ...
                    switch (cellType) {
                        ...
                        case 3:
                            if (!isEnd && (placeIndex < places.size())) {
                                ...
                                curTransportView = getTransportView(preferTransportType, curTravel, 0, strDepartureTime, placeIndex);
                        ...
                        case 9:
                            if (placeIndex < places.size()) {
                                strNextPlaceId = places.get(placeIndex).getId();
                                curTravel = Utility.getTravel(travels, strCurrentPlaceId, strNextPlaceId);
                                String startTime = Utility.computeTime(strPreTime, times.get(placeIndex));
                                curTransportView = getTransportView(preferTransportType, curTravel, 1, strDepartureTime, placeIndex);
                        ...
                        case 13:
                            if (!isEnd && (placeIndex < places.size())) {
                                ...
                                curTravel = Utility.getTravel(travels, strCurrentPlaceId, strNextPlaceId);
                                String startTime = Utility.computeTime(strPreTime, times.get(placeIndex));
                                curTransportView = getTransportView(preferTransportType, curTravel, 2, strDepartureTime, placeIndex);
                        ...
                        case 15:
                            if (!isEnd && (placeIndex < places.size())) {
                                ...
                                curTravel = Utility.getTravel(travels, strCurrentPlaceId, strNextPlaceId);
                                String startTime = Utility.computeTime(strPreTime, times.get(placeIndex));
                                curTransportView = getTransportView(preferTransportType, curTravel, 3, strDepartureTime, placeIndex);
    ...
    private TransportView getTransportView(int transType, Travel travel, int direction, String mDepartureTime, final int mPlaceIndex ) {
        boolean isCar = true;
        switch (transType) {
            case 1: // car
                isCar = true;
        ...
        TransportView view = new TransportView(activity, travel, direction, mDepartureTime, strArrivalTime, info, isCar);

        view.getTvFrom().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Get Current Time
                ...
                TimePickerDialog timePickerDialog = new TimePickerDialog(activity,
                        new TimePickerDialog.OnTimeSetListener() {
                            ...
                            public void onTimeSet(TimePicker view, int hourOfDay,
                                ...
                                activity.getIntent().putExtra("CHANGE_DEPARTURE_TIME", hourOfDay + ":" + minute);
                                activity.getIntent().putExtra("PLACE_INDEX", mPlaceIndex);
                                activity.finish();
        ...
        view.getTvTo().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Get Current Time
                final Calendar c = Calendar.getInstance();
        ...
        if (direction == 2) {
            view.getTvTo().setClickable(true);
            view.getTvFrom().setClickable(false);
        } else {
            view.getTvTo().setClickable(false);
            view.getTvFrom().setClickable(true);
        }


        strDepartureTime = Utility.computeTime(strArrivalTime, times.get(mPlaceIndex));
    ...
    private void showPopupMenuPlace(PlaceView v) {
        ...
        menu.getMenu().getItem(0).setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                TourDiagramFragment_bk.this.activity.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(strSite)));
                return true;
            }
        });
